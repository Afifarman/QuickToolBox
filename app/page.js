...
function ToolGrid({ items, base }) {
  return <div className="grid">{items.map(([slug,icon,title,desc], i) => {
    ...
    return <Link className="card" href={href} key={`${slug}-${i}`}><i>{icon}</i><div><h3>{title}</h3><p>{desc}</p></div><strong>→</strong></Link>;
  })}</div>;
}
...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section id="ai"><small>AI & CV</small><h2>AI tools without duplicates.</h2><ToolGrid items={aiTools} base="ai"/></section>
